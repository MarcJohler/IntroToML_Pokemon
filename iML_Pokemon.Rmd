...
author: "Hyeyoung Park", "Marc Johler", ""
date: "`r Sys.Date()`"
output: pdf_document
...
https://awesomeds.shinyapps.io/iMLproject/
...
param_set <- ParamSet$new(
  params = list(ParamDbl$new("alpha",lower=0.8,upper=0.9),
                ...
                ParamDbl$new("eps",lower = 0.0000000004,upper=0.00005)
  )
)
terminator <- term("none")
tuner <- tnr("grid_search")
learner_tunedglmnet <- AutoTuner$new(learner_glmnet,resample_inner,measures,param_set,terminator,tuner)
...
#####eps is the  value that specifies an epsilon-range to replace zero or close to zero probabilities
by threshold. It applies to metric variables.
...
terminator <- term("none")
tuner <- tnr("grid_search")
learner_tunednb <- AutoTuner$new(learner_nb,resample_inner,measures,param_set,terminator,tuner)
...
terminator <- term("none")
tuner <- tnr("grid_search")
learner_tunedkknn <- AutoTuner$new(learner_kknn,resample_inner,measures,param_set,terminator,tuner)
...
percentage_plot <- function(pokemonA,pokemonB){
  ...
  prediction <- learner_tunedkknn$predict_newdata(newdata=data_for_classification)
