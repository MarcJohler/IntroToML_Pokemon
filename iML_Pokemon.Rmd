---
title: "iML with Pokemon"
author: "Hyeyoung Park, Marc Johler, Mohamed Bechir Ghannouchi"
date: "13/05/2020"
output: html_document
---
<center><img src="https://pics.me.me/pokemon-go-protective-mask-pikachu-special-event-70613775.png" width="200"/>
</center>
...
## 2. Read Datasets 
original datasets from Kaggle: [Here](https://www.kaggle.com/terminus7/pokemon-challenge#combats.csv) 
...
## 3. Descriptive Statistics : EDA Pokemon

Visit ShinyApp:

[Visit My Shiny App for Pokemon EDA](https://awesomeds.shinyapps.io/iMLproject/)
...
table(daten$Firste_pokemon_faster,First_wins)
...
```{r tuning logReg, results='hide'}
###Classification via logistic regression with family set to "binomial".The parameters tuned are the maxit(number of iterations) and epsilon
...
```{r benchmark Viz, echo=FALSE}
# Benchmark Boxplot
print(bmr$aggregate(measures))
autoplot(bmr,measure = msr("classif.acc")) + theme_bw() + theme(axis.text.x = element_text(angle = 45, hjust =1))
autoplot(bmr$clone()$filter(task_id = "task_combat"), type = "roc") 
```
...
# Visulisation : The probability for first Pokemon to win via gauge graph 
